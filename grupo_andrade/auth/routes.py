...
def register():
    ...
    if form.validate_on_submit():
        ...
        user = User(username=form.username.data, email=form.email.data, password=hashed_password, data_criacao=datetime.utcnow(), rg=form.rg.data, cpf_cnpj=form.cpf_cnpj.data)
        db.session.add(user)
        db.session.commit()
